"""Bridges to the ray tracer and to vecdiff's own field containers.

Nothing in the wave core imports these at module scope; they are only pulled
in when you call them, so the subpackage works with no extra dependencies.

* :func:`system_from_raytracer` turns a traced objective into a
  :class:`~vecdiff.wave.systems.LithoSystem` (prescription -> exit-pupil
  wavefront).  It needs the optional ``geometrical-raytracer`` package.
* :func:`to_vecdiff_field` hands a :class:`~vecdiff.wave.fields.Field` to
  the host package's polarization-ellipse plots.
"""
...
def plot_polarization(field: Field, ax=None, **kwargs):
    ...
    """
    from ..polarization_visualization import plot_field_polarization  # noqa: PLC0415
