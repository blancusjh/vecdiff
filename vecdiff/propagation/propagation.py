...
def spectrum_of(field, *, direction=1):
    ...
    a = np.stack((ax, ay, az), axis=-1)*np.exp(-1j*k @ frame.origin)[:, None]
...
be exponentially ill-conditioned; this routine never silently filters them.
    ...
    coefficients[:, iy, ix] = (spec.amplitudes*np.exp(1j*spec.wavevectors @ domain.frame.origin)[:, None]).T*grid.period_area
