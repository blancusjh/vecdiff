...
- Las energías de canal (`f_cross`, transmisión) se calculan **en el espacio
  de pupila vía Parseval** (`∫|𝓗ₘ[f]|² q dq = ∫|f|² r dr`): la integración
  numérica en q con grillas alcanzables es oscilatoria y no converge (véase
  nota abajo).
...
```bash
../../../.venv/bin/jupytext --to ipynb --execute 01_baseline_spot_metrics.py
```
