...
def incident_wave(angle, polarization):
    ...
    return plane_wave(
        (np.sin(t), 0, np.cos(t)), e, wavelength=wavelength, medium=Medium(n1)
    )
...
for ax in axes:
    ...
    ax.axvline(
        theta_c, color="black", ls=":", label="Critical" if ax is axes[0] else None
    )
...
axes[0].set(
    ...
    ylim=(-0.02, 1.02),
    title="Reflected and transmitted power",
)
...
print(
    "Maximum normalized boundary jumps:",
    {key: max(row[key] for row in jumps) for key in jumps[0]},
)
...
def beam(angle, pol="s", count=161, waist=2.0):
    ...
    weights = np.exp(-((k1 * waist * delta) ** 2) / 4)
    weights /= weights.sum()
    directions = np.c_[np.sin(theta), 0 * theta, np.cos(theta)]
    polarization = (
        np.tile([0, 1, 0], (count, 1))
        if pol == "s"
        else np.c_[np.cos(theta), 0 * theta, -np.sin(theta)]
    )
    return ElectricSpectrum(
        k1 * directions, weights[:, None] * polarization, wavelength, Medium(n1)
    )
...
cases = [
    ...
    (theta_c, "s", "Critical-centred beam"),
...
for ax, (angle, pol, title) in zip(axes.flat, cases):
    ...
    ei, hi = incoming.evaluate(points[lower])
    er, hr = out.reflected.evaluate(points[lower])
    et, ht = out.transmitted.evaluate(
        points[~lower]
    )  # Never continue growing evanescent waves into z<0.
    e[lower] = ei + er
    e[~lower] = et
    scalar_map(
        fig,
        ...
        x,
        z,
        f"{title}: {angle:.2f}°, {pol}",
        vmax=4,
    )
    ax.axhline(0, color="cyan", lw=1)
    ax.text(-11, 6.6, "air: n=1", color="white")
...
axes[0].semilogy(
    zz[::15], np.exp(-2 * kappa * zz[::15]), "o", mfc="none", label=r"$e^{-2\kappa z}$"
)
axes[0].axvline(1 / (2 * kappa), color="gray", ls=":")
axes[0].legend()
axes[0].set(
    xlabel="Distance into air (µm)",
...
for incoming, out, _ in beam_results:
    ...
    et, ht = out.transmitted.evaluate(probe)
    finite_jumps.append(
        boundary_residuals(
            ei + er,
            hi + hr,
...
axes[1].set(
    ...
    title="Finite-beam Maxwell boundary conditions",
)
...
def physical_field(wave, probe):
    out = interface_transform(wave, interface)
    e = np.empty_like(probe, dtype=complex)
    mask = probe[:, 2] < 0
...
scalar_map(
    fig,
    axes[0],
    np.sum(abs(zoom_e) ** 2, axis=-1),
    zoom_x,
    zoom_z,
    "TIR near field: electric norm",
    vmax=4,
)
scalar_map(
    fig,
