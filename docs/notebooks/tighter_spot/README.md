...
| `03_quabis_dioptrio` | El mismo experimento a través del **dioptrio estigmático** (apodización de Fresnel `t_p, t_s` exacta del óvalo, canales de `spot_tools`), con las iluminaciones uniformes de la tesis: escalar de referencia en trazos negros, lineal, circular, radial y radial con máscara anular `ρ' ≥ 0.9a` (que juega el papel de la iluminación anular de Quabis). Primero mapea la cota rasante `sinθ_max = sin αᵢ(r_g)` (función solo de `n_i` y `|z_0|/z_i`); luego compara el dioptrio de vidrio de la tesis llevado al 97 % de su radio rasante (`n_i=1.5`, `z_0=−10` mm, `sinθ_max=0.56`) con uno de objeto lejano (`n_i=2.4`, `z_0=−50` mm, `sinθ_max=0.79`), ambos con `z_i=6` mm. Barrido de apertura y validación del canal `H₀` cerrado contra `generate_Ez_cartesian` (4.7e-3). | **La cota rasante decide el régimen**: el vidrio no pasa de `sinθ_max≈0.74` ni con objeto en el infinito — nunca alcanza el régimen de Quabis. El dioptrio de vidrio a su apertura máxima practicable (0.56) queda en el orden convencional: radial anular 1.033 λ² > escalar 0.433 λ², aunque `E_z` ya llena el centro (región de mitad de altura en disco, no corona). Con objeto lejano (0.79) la jerarquía se invierte: radial anular (0.068 λ²) < **escalar** (0.070 λ²) < lineal ≈ circular (0.080) < radial (0.300 λ²) — el mismo orden que el artículo a NA=0.9. El cruce ocurre en `sinθ_max ≈ 0.78`: el efecto de Quabis **sobrevive a la apodización de Fresnel** del dioptrio. |
...
apodización (`√cosθ` aplanática vs. `t_p` de Fresnel, esta última acompañada
del factor geométrico `A(Q)`). El ángulo del rayo imagen es
`sinθ = r/ℓᵢ(r)`, no `r/z_i`: esa segunda forma es su aproximación de
superficie delgada y en `r = 2` mm difiere un 25 %. El dioptrio de vidrio de
los ejemplos (`sinθ_max ≈ 0.56`, NA_i ≈ 0.84) queda en el régimen «anillo
dominante» (`I_z(0) ≈ 0.47·max I_⊥`): el cruce al régimen de Quabis
(`E_z` dominante) ocurre en `sinθ_max ≈ 0.78` (cuaderno 03), inalcanzable
para el vidrio por la cota rasante del óvalo. Los resultados del cuaderno 03
están integrados en la tesis (capítulo III, subsección «Punto focal para
diferentes polarizaciones y aperturas», y Conclusiones).

## Cómo regenerar

```bash
uv run jupytext --to ipynb --execute 01_quabis2000_replication.py
uv run jupytext --to ipynb --execute 02_quabis2000_vecdiff.py
uv run jupytext --to ipynb --execute 03_quabis_dioptrio.py
```
