...
# grande para que las colas truncadas no «resuenen» de vuelta al centro) y
# $E_z$ se reconstruye con `generate_Ez_cartesian`. La validación compara
# $|E_z|$ contra el canal de Richards–Wolf:
# $2\,I_1\sin\varphi$ (lineal $\parallel\hat{\mathbf y}$) e $I_z$ (dona).
...
Ey = onto_window(p, "I0") - onto_window(p, "I2") * np.cos(2.0 * phi_w)
Ex = onto_window(p, "I2") * np.sin(2.0 * phi_w)
...
# %% [markdown]
# Cada fila comparte escala de color (la fracción «máx» de cada panel es
# relativa al máximo de su fila). En la fila lineal, la cruzada $|E_x|^2$
# —cuatro lóbulos— y la longitudinal $|E_z|^2$ —dos lóbulos sobre el eje de
# polarización— son las mismas firmas vectoriales del capítulo III de la
...
for (row_name, panels), ax_row in zip(rows.items(), axes):
    ...
    for ax, (title, m) in zip(ax_row, panels):
        img = m[np.ix_(sel, sel)] / vmax
        im = ax.imshow(img, extent=ext, origin="lower", cmap="hot",
                       vmin=0.0, vmax=1.0, aspect="equal")
        ax.set_title(title, fontsize=10)
        ax.text(0.03, 0.97, f"máx = {float(np.max(img)):.3f}",
