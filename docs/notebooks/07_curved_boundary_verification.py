...
for ax, values, title in zip(
    axes,
    [
        np.sum(abs(physical) ** 2, axis=-1),
        abs(physical[..., 2]) ** 2,
        np.sum(abs(reflected) ** 2, axis=-1),
    ],
    [
        "Total physical electric field",
        "Longitudinal electric component",
        "Reflected field below the cap",
    ],
):
    scalar_map(
        fig,
...
report = json.loads(
    (root / "benchmarks/results/curved_boundary_limits.json").read_text()
)
rows = report["cases"]
saved = rows[-1]
...
np.testing.assert_allclose(
    ...
    list(saved["extrapolated_boundary_residuals"].values()),
    rtol=2e-5,
    atol=2e-8,
)
...
for key, label in [
    ...
    axes[0].loglog(
        fresh["offsets_over_wavelength"],
        [r[key] for r in fresh["offset_boundary_residuals"]],
        "o-",
        label=label,
    )
...
for indices in [[1.0, 1.5], [1.5, 1.0]]:
    subset = [
        ...
        and r["illumination"] == "plane"
        and not r["flat_control"]
        and r["incidence_degrees"] == 0
    ]
    axes[1].loglog(
        [r["radius_over_wavelength"] for r in subset],
        [max(r["extrapolated_boundary_residuals"].values()) for r in subset],
        "o-",
...
axes[0].set(
    ...
    title="Localized beam: approach from both sides",
)
axes[1].axhline(0.01, color="k", ls="--", label="1% amplitude criterion")
axes[1].set(
    xlabel=r"$R/\lambda_0$",
...
for j, key in enumerate(["tangential_E", "tangential_H", "normal_D", "normal_B"]):
    ax.bar(
        ...
        [100 * r["extrapolated_boundary_residuals"][key] for r in controls],
        width=0.18,
        label=key.replace("_", " "),
    )
...
ax.set(
    ...
    title=r"Controls at $R=10\lambda_0$, aperture $5\lambda_0$",
)
...
for label, row in zip(labels, controls):
    print(
        ...
        row["numerical_convergence_passed"],
    )
# %% [markdown]
...
for n2, label in [(1.0, "Equal-index control"), (1.5, "Curved dielectric")]:
    ...
    for ax, key in zip(
        axes,
        [
            "extrapolated_boundary_residuals",
            "local_amplitude_normalized_boundary_residuals",
        ],
    ):
        ax.semilogy(
            [r["probe_radius_over_radius"] for r in subset],
            [100 * max(r[key].values()) for r in subset],
            "o-",
            label=label,
        )
...
for ax in axes:
    ax.axhline(1, color="k", ls="--", lw=1)
    ax.set(
        xlabel=r"Probe radius $\rho/R$",
        ylabel="Largest normalized amplitude mismatch (%)",
    )
    ax.legend()
    ax.grid(alpha=0.2)
...
for row in scan:
    if row["indices"][1] == 1.5:
        print(
            f"ρ/R={row['probe_radius_over_radius']:.1f}: incident |E|={row['incident_electric_norm_at_probe']:.4f}, local normalized residual={100 * max(row['local_amplitude_normalized_boundary_residuals'].values()):.3f}%"
        )
