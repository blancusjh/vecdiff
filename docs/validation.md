...
Jupyter execution. Run the commands above locally to execute the nine maintained
application notebooks, check their embedded PNG figures, and exercise the seven
maintained example workflows. Executed outputs are committed in the source-tree
notebooks. Historical runs that included the removed solver are not current suite
results.
